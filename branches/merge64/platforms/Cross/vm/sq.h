...
/* filename copy/transform macro. An opportunity to transform the filenames for
   platforms with strange needs, and a simple encapsulation for everyone else
*/
...
/* typedef and macros to handle primitive dispatch through an array of function addresses
 */
typedef sqInt (*fptr)();
...
typedef struct sqMenuEvent {
	...
	int windowIndex; /* SmallInteger identifying host window structure */
 } sqMenuEvent;
